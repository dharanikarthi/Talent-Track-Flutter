...
from app.processing.verticaljump import process_verticaljump
...
def test_process_verticaljump_outputs(tmp_path: Path):
    vid = tmp_path/"dummy.mp4"
    _make_dummy_video(str(vid))
    res = process_verticaljump(str(vid), str(tmp_path))
